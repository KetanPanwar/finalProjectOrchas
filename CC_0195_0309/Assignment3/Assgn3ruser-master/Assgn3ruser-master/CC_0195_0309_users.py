        ...
        if(resp_send.status_code == 400):
            ...
            if match:
                ...
                resp_send = requests.post(
                    "http://34.194.124.13:80/api/v1/db/write", json=data)
                return jsonify({}), 201
            else:
                return jsonify({}), 400
        else:
...
#     data = json(d)
    print("hey")
    resp_send = requests.post("http://34.194.124.13:80/api/v1/db/read", json=data)
    print("hi")
...
def remove_user(username):
    ...
    if request.method == 'DELETE':
        ...
        resp_send = requests.post(
            "http://34.194.124.13:80/api/v1/db/read", json=data)
        if(resp_send.status_code == 400):
            return jsonify({}), 400
