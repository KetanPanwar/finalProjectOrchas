...
def startup():
	...
	client.containers.run("worker:latest", name='master', command=["python3 worker.py 0","service mongodb start"], detach=True)
	...
	salveno+=1
	client.containers.run("worker:latest", name='slave'+str(salveno),command=["python3 worker.py 1","service mongodb start"], detach=True)
